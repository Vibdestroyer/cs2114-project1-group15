...
public class Restaurant extends Encounter {
    ...
    public Restaurant() {
        ...
        if (this.takesOnlineOrders) {
            timeChange -= 4;
            resultText +=
                " The restaurant takes online orders which saved time.";
        }

        if (this.grabAndGo) {
            timeChange -= 4;
            resultText +=
                " The restaurant served grab and go food which saved time.";
        }

        int distanceChange = 3;

        Choice getFood = new Choice("Get food from restaurant", resultText
            + " This took " + timeChange
            + " minutes and gave you the energy to travel 0.3 miles.",
